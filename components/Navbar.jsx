...
export default function Navbar() {
    useEffect(() => {
        ...
        const updateNavbarColor = () => {
            ...
            // Orange name only while over DiagonalScroll dark cards
            const dsTrack = document.querySelector('.ds-track');
            if (dsTrack) {
                const dsTop = dsTrack.getBoundingClientRect().top + window.scrollY;
                const dsBottom = dsTop + dsTrack.offsetHeight;
                if (scrollPos >= dsTop && scrollPos < dsBottom) {
                    navbar.classList.add('over-diagonal');
                } else {
                    navbar.classList.remove('over-diagonal');
                }
            }
        };
        ...
        const waTrigger = document.querySelector('.nav-wa-trigger');
        ...
        if (waTrigger && waBox) {
            ...
            const onLeaveRight = () => {
                ...
                gsap.to(waBox, { scale: 0, opacity: 0, duration: 0.3, ease: 'expo.in', delay: 0.05, onComplete: () => gsap.set(waBox, { visibility: 'hidden' }) });
            };

            waTrigger.addEventListener('mouseenter', onEnterRight);
            waTrigger.addEventListener('mouseleave', onLeaveRight);
            cleanups.push(() => {
                waTrigger.removeEventListener('mouseenter', onEnterRight);
                waTrigger.removeEventListener('mouseleave', onLeaveRight);
            });
        }

        // ─── Navbar Right (CV) Hover ───
        const cvTrigger = document.querySelector('.nav-cv-trigger');
        const cvBox = document.querySelector('.nav-cv-box');
        const cvIcon = document.querySelector('.nav-cv-icon');

        if (cvTrigger && cvBox && cvIcon) {
            const cvInner = cvBox.querySelector('.nav-popout-inner');
            const cvItems = cvInner ? Array.from(cvInner.children) : [];

            gsap.set(cvBox, { visibility: 'visible', scale: 1, opacity: 1 });
            const cvBoxRect = cvBox.getBoundingClientRect();
            const cvIconRect = cvIcon.getBoundingClientRect();
            const cvOriginX = (cvIconRect.left + cvIconRect.width / 2) - cvBoxRect.left;
            const cvOriginY = (cvIconRect.top + cvIconRect.height / 2) - cvBoxRect.top;
            const cvOrigin = `${cvOriginX}px ${cvOriginY}px`;

            gsap.set(cvBox, { visibility: 'hidden', scale: 0, opacity: 0, transformOrigin: cvOrigin });
            gsap.set(cvItems, { y: 10, opacity: 0 });

            const onEnterCV = () => {
                gsap.killTweensOf(cvBox);
                gsap.killTweensOf(cvItems);
                showOverlay();
                gsap.to(cvIcon, { color: '#38bdf8', duration: 0.3 });
                gsap.set(cvBox, { visibility: 'visible' });
                gsap.fromTo(cvBox, { scale: 0, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.8, ease: 'expo.out' });
                gsap.to(cvItems, { y: 0, opacity: 1, duration: 0.45, stagger: 0.06, ease: 'power3.out', delay: 0.18 });
            };
            ...
            const onLeaveCV = () => {
                ...
                gsap.killTweensOf(cvItems);
                hideOverlay();
                gsap.to(cvIcon, { duration: 0.3, onComplete: () => gsap.set(cvIcon, { clearProps: 'color' }) });
                gsap.to(cvItems, { y: 10, opacity: 0, duration: 0.15, ease: 'power2.in' });
                gsap.to(cvBox, { scale: 0, opacity: 0, duration: 0.3, ease: 'expo.in', delay: 0.05, onComplete: () => gsap.set(cvBox, { visibility: 'hidden' }) });
            };

            cvTrigger.addEventListener('mouseenter', onEnterCV);
            cvTrigger.addEventListener('mouseleave', onLeaveCV);
            cleanups.push(() => {
                cvTrigger.removeEventListener('mouseenter', onEnterCV);
                cvTrigger.removeEventListener('mouseleave', onLeaveCV);
            });
        }
            ...
            <nav className="navbar">
                    ...
                    <div className="nav-hover-trigger">
                        ...
                        <div className="nav-popout nav-work-box">
                            <div className="nav-popout-inner">
                                <a href="https://www.proeliumacademy.in" target="_blank" rel="noopener noreferrer" className="nav-work-item">
                                    <div className="nav-work-item__img-wrap">
                                        <img src="/assets/nav-work-proelium.jpg" loading="eager" alt="Proelium Academy website" className="nav-work-item__img" />
                                    </div>
                                    <div className="nav-work-item__text">
                                        <span className="nav-work-badge badge-maroon">web dev</span>
                                        <h4 className="nav-work-title">proelium academy website</h4>
                                    </div>
                                </a>
                                <div className="nav-work-item">
                                    <div className="nav-work-item__img-wrap">
                                        <img src="/assets/nav-work-hardening.avif" loading="eager" alt="System Hardening Tool" className="nav-work-item__img" />
                                    </div>
                                    <div className="nav-work-item__text">
                                        <span className="nav-work-badge badge-pink">cybersecurity</span>
                                        <h4 className="nav-work-title">multi-platform system hardening tool</h4>
                                    </div>
                                </div>
                                <div className="nav-work-item">
                                    <div className="nav-work-item__img-wrap">
                                        <img src="/assets/nav-work-sih.avif" loading="eager" alt="Smart India Hackathon" className="nav-work-item__img" />
                                    </div>
                                    <div className="nav-work-item__text">
                                        <span className="nav-work-badge badge-pink">hackathon</span>
                                        <h4 className="nav-work-title">SIH 2025 — national runner-up</h4>
                                    </div>
                                </div>
                                <a href="https://github.com/harishvidyarthcsecs" target="_blank" rel="noopener noreferrer" className="nav-work-btn"><span className="nav-work-btn__text">All projects on GitHub</span></a>
                            </div>
                        </div>
                    </div>
                </div>
                <div className="nav-center" style={{ cursor: "url('/assets/Cursor SVG/cursor-pointer.svg') 12 12, pointer" }}>
                    <span className="logo-truus">Harish Vidyarth</span>
                </div>
                <div className="nav-right" style={{ cursor: "url('/assets/Cursor SVG/cursor-pointer.svg') 12 12, pointer" }}>
                    ...
                    <div className="nav-hover-trigger nav-cv-trigger">
                        ...
                        <div className="nav-popout nav-cv-box">
                            <div className="nav-popout-inner">
                                <h4 className="nav-wa-title">résumé</h4>
                                <p className="nav-wa-desc">Harish's latest CV — one click download.</p>
                                <a href="/Harish_Vidyarth_CV.pdf" download className="nav-wa-link">
                                    <span className="nav-wa-link-text">Download CV / Résumé</span>
                    ...
                    <div className="nav-hover-trigger nav-wa-trigger">
                        ...
                        <div className="nav-popout nav-wa-box">
                            <div className="nav-popout-inner">
                                <h4 className="nav-wa-title">get in touch</h4>
                                <p className="nav-wa-desc">Reach out via WhatsApp or email — always happy to connect.</p>
                                <a href="https://wa.me/916382551983" target="_blank" rel="noopener noreferrer" className="nav-wa-link">
                                    <span className="nav-wa-link-text">+91 – 6382551983</span>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="100%" viewBox="0 0 169 10" fill="none" className="draw-btn__svg nav-wa-link-svg">
                                        <path d="M1 6.5661C56.3941 3.06082 112.187 1.20095 168 0.999878" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.25" />
                                        <path d="M32.1313 8.63371C68.2147 6.92799 104.462 6.13378 140.695 6.25107" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.25" />
                                    </svg>
